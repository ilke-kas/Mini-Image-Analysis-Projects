...
table_1 = part4a(contrast1_new, contrast2_new,tolerances,p0,"SSE");
table_2 = part4a(contrast1_new, contrast2_new,tolerances,p1,"SSE");

table_3 =part4a(contrast1_new, contrast2_new,tolerances,p0,"myNCC");
table_4 =part4a(contrast1_new, contrast2_new,tolerances,p1,"myNCC");
...
%% 4b- scalings
scalings = [1,1.5,2,2.5];
table_5 = part4b(contrast1_new, contrast2_new,scalings,p0,"SSE")
table_6 = part4b(contrast1_new, contrast2_new,scalings,p1,"SSE")
%%
table_7 = part4b(contrast1_new, contrast2_new,scalings,p0,"myNCC")
table_8 = part4b(contrast1_new, contrast2_new,scalings,p1,"myNCC")
...
%% 4-scaling testing
scalings = [1,1.5,2,2.5];
table_9 = scaletesting(contrast1_new, contrast2_new,scalings,p0,"SSE")
table_10 = scaletesting(contrast1_new, contrast2_new,scalings,p1,"SSE")
table_11 = scaletesting(contrast1_new, contrast2_new,scalings,p0,"myNCC")
table_12 = scaletesting(contrast1_new, contrast2_new,scalings,p1,"myNCC")

table_part_c_cell = [table_9;table_10;table_11;table_12]
table_part_c = cell2table(table_part_c_cell, ...
    "VariableNames",["Similarity Method" "Initializing Point" "Scaling" "Iteration Number" "Optimal Value" "Error"])


function for_table = scaletesting(contrast1_new, contrast2_new, scaling, p, similarity_funct)
    for_table = {}
    ...
    for i=1:length(scaling)
        ...
        [phat, fval, exitflag,output] = fminsearchstep(scaling(i),@funregister,p,options,contrast1_new,contrast2_new,v3,similarity_funct)
        phats(i,1) = phat(1);
        phats(i,2) = phat(2);
      ...
      showImages(all_images,all_labels,'Q4_Resulting_Images',"C-Testing scaling Images with different scaling  inital p: " + p(1)+","+p(2)+" with " + similarity_funct + "step p: "+ phat(1)+","+phat(2), 4,4);
end
...
function for_table = part4a(contrast1_new, contrast2_new, tolerances, p, similarity_funct)
    ...
    for i=1:length(tolerances)
        images = {};
        lables = {};
        video_name = "Q4optimize_"+similarity_funct+"_"+p(1)+"_"+p(2)+"_"+tolerances(i)+".avi"
        v3 = VideoWriter('Q4_Resulting_Videos/'+video_name)
        v3.FrameRate = 1;
        open(v3)
        %subtracted image without registration
        ...
        images = { contrast1_new,  image2_registered,image_subtracted_without_reg, image2_subtracted}
        labels = {
        "Contrast 1", "Contrast 2 registered with " + similarity_funct+ " inital p: " + p(1)+","+p(2)+",tol:"+tolerances(i)+" error: " + error,"Subtracted image without registration",  "Subtracted image with registration"+tolerances(i)};
        writeToVideo(image2_subtracted , v3,"Final Optimal One p: ("+ phat(1)+","+phat(2)+")"  )
        close(v3);
    ...
    showImages(all_images,all_labels,'Q4_Resulting_Images',"A- Images with different tolerance inital p: " + p(1)+","+p(2)+" with " + similarity_funct, length(tolerances),4);
end
